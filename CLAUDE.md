...
Testing (both sides, run from `frontend/` or `backend/`): `npm test` (Vitest, single run — what CI runs) / `npm run test:watch`. Unit tests are colocated `src/**/*.test.ts` with fixture builders in `src/test-utils/quote-builders.ts`; they cover the quote money math (`quote-calculations`), Excel row layout (`buildExcelRows`), and the backend clone/mapper/zod pipeline — all pure, no DB or env needed. CI (`.github/workflows/test.yml`) runs tests + build + lint on every push/PR to `main`/`testing`/`prod`. See `docs/testing.md` for conventions (business-rule test naming, builder overrides, known-gap pins).
...
- Auth: Supabase Auth. Frontend (`@supabase/supabase-js`) drives login/logout/change-password directly against Supabase; backend verifies the resulting JWT (`jose`, ES256, against Supabase's JWKS — `SUPABASE_URL`, cached in memory after first fetch, not re-fetched per request) via `require-auth.ts` middleware on every `/api/*` route, attaching `req.authUserId` (the Supabase auth UID). This project's on Supabase's newer asymmetric signing-keys setup, not the legacy shared HS256 secret. `require-auth.ts` answers **503, not 401**, when verification itself is unavailable (JWKS timeout / no matching key) — collapsing that into 401 made a transient blip indistinguishable from a signed-out user, which the frontend rendered as an empty folder list. `api-client.ts` gets its bearer token from `getAccessToken()` (`supabase-client.ts`), which **awaits `supabase.auth.getSession()`** rather than reading a variable cached by an `onAuthStateChange` listener: the listener fires asynchronously after Supabase rehydrates/refreshes the stored session, so requests issued during that window (notably the folder/category preload on mount) went out with no `Authorization` header and 401'd. Keep the token read awaited — a synchronous cache reintroduces the race. Only two users ever exist per Supabase project — see "Environments" above. There's no backend user-profile table — display name/email come straight from the Supabase session (`useCurrentUser()` in `auth-context.tsx`), and `Folder`/`MasterTemplate` ownership is keyed directly off the auth UID, so nothing needs a DB-side link to "the user".
...
**A failed load must never render as an empty state.** `category-library-context.tsx`'s preload wraps its fetch in `try/catch/finally` and exposes `error` + `reload()` alongside `loading`; `Folders.tsx` and `CategoryList.tsx` render `<LoadErrorState message onRetry>` (`src/components/LoadErrorState.tsx`) when `error` is set. Before this, the preload had no `catch`: a rejection skipped `setLoading(false)` entirely, leaving `folders`/`categories` empty — so users saw a stuck spinner or, worse, a convincing "No categories yet" for data that existed. In `CategoryList.tsx` the `error` branch must stay **above** the `!folder` check, since an empty `folders` array would otherwise render "Folder not found." Apply the same shape to any new preload.
...
- Three tabs (`basic` / `items` / `summary`), rendered by `QuoteEditorLayout`, are local `useState` there, not routes: `BasicInformationTab.tsx`, `QuotationItemsTab.tsx` (section list + accordion of areas-of-work, each with a `LineItem[]` table), `SummaryTab.tsx` (read-only rollup, also reused inside `QuotePreview.tsx`'s full-screen overlay).
  ...
  - **Collapsible side panels**: `QuoteEditorLayout.tsx`'s tab rail (Basic Information/Quotation Items/Summary) and `QuotationItemsTab.tsx`'s sections sidebar can each collapse to a `w-12` icon/letter-only rail to free horizontal width for the line-item tables. Toggle button shows a chevron + "Minimise" label when expanded, icon-only chevron when collapsed. Collapse state persists per-panel across reloads via `useLocalStorageState` (`src/hooks/use-local-storage-state.ts`, keys `qms:tabRailCollapsed`/`qms:sectionsSidebarCollapsed`) — the first `localStorage`-backed state in the frontend, used only for this UI preference (not server-owned data, so it bypasses the `*-service.ts` pattern above by design).
  - Both the section-level description (under the section header) and each line item's description cell use `Textarea` (not `Input`) so multi-line content is fully visible rather than clipped — `Textarea`'s `field-sizing-content` auto-grows with content.
  - **Read-only output column contract** — `SummaryTab.tsx`, `QuotePrintDocument.tsx` and `quote-excel-export.ts` share one line-item column set: `No. · Description · Qty · Unit · Unit Price · Amount`. `Qty` and `Unit` are separate cells (they used to be one `{qty} {unit}` cell), `Unit Price` is `item.selling`, `Amount` is `itemTotal(item)` (literal `FOC` when `item.foc`). Change one of the three and change all three — the Excel test (`quote-excel-export.test.ts`) pins the column count and offsets, so section/total rows there pad to the right column index. `QuotationItemsTab.tsx`'s editable table is deliberately **not** part of this contract and keeps its own columns.
  - **Currency lives in the header, not the cells** — headers read `Unit Price ({currencySymbol})` / `Amount ({currencySymbol})` and cells render bare numbers (`toLocaleString('en-SG', { min/maxFractionDigits: 2 })` in the two React tables, raw `number` in Excel so amounts stay numeric and sortable — including payment-term amounts). `formatMoney()` is still used for the prefixed `S$…` display in Summary's totals/metric boxes, just not inside these tables. `exportQuoteToExcel()` derives the last two `!cols` widths from the currency-aware header strings so a longer symbol doesn't clip.
  - **Summary-only cost/profit columns** — `SummaryTab.tsx` alone (never the client-facing print/Excel output) replaces `Unit Price` with `Cost Price · Seller Price · Amount · Profit/Loss (%)` (`item.cost`, `item.selling`, `itemTotal`, `itemProfitPercent`), and renders `Total Cost` / `Total Price` / `Profit / Loss` `MetricBox`es above Payment Terms, summed over `sectionTotals()` of each included section. The Profit/Loss box shows amount plus margin over total cost, e.g. `S$14,682.00 (34.00%)`.
  - **Single-line headers** — the output tables set `whitespace-nowrap` on header/numeric cells and drop the old fixed `w-*` column widths (which forced two-line headers); Summary's table is wrapped in an `overflow-x-auto` + `min-w-max` container so wide column sets scroll horizontally instead of squeezing.
