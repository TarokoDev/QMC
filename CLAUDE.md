...
- Three tabs (`basic` / `items` / `summary`), rendered by `QuoteEditorLayout`, are local `useState` there, not routes: `BasicInformationTab.tsx`, `QuotationItemsTab.tsx` (section list + accordion of areas-of-work, each with a `LineItem[]` table), `SummaryTab.tsx` (read-only rollup, also reused inside `QuotePreview.tsx`'s full-screen overlay).
  ...
  - **Collapsible side panels**: `QuoteEditorLayout.tsx`'s tab rail (Basic Information/Quotation Items/Summary) and `QuotationItemsTab.tsx`'s sections sidebar can each collapse to a `w-12` icon/letter-only rail (chevron toggle button) to free horizontal width for the line-item tables. Collapse state persists per-panel across reloads via `useLocalStorageState` (`src/hooks/use-local-storage-state.ts`, keys `qms:tabRailCollapsed`/`qms:sectionsSidebarCollapsed`) — the first `localStorage`-backed state in the frontend, used only for this UI preference (not server-owned data, so it bypasses the `*-service.ts` pattern above by design).
